...
export async function getDashboardStats() {
  ...
  const eligibleRoiInvestments = investments.filter(
    (inv) =>
  ...
  if (eligibleRoiInvestments.length > 0) {
    const totalEligibleInvestment = eligibleRoiInvestments.reduce(
      (sum, inv) => sum + inv.amountPaid,
      0,
    );
  ...
  return {
    ...
    portfolioItems: investments.map((inv) => {
      ...
      const paidSchedules = inv.schedules.filter(
        (s) => s.status === "PAID",
      ).length;
      ...
      return {
        ...
        status: inv.status,
        expectedRoi: isEligibleForRoi
          ? inv.selectedOption.project.expectedRoiPercent
          : 0,
