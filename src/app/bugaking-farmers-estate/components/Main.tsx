...
//   const [plan, setPlan] = useState<"outright" | "monthly" | "weekly">(
//     "outright",
//   );
...
//     try {
//       // 1. Send the lead data to your Next.js API route (which forwards to Google Sheets)
...
//           plan,
//           units: plots,
//           estateName: "BugaKing Farmers Estate Kuje", // Identifies this lead in your CRM
//         }),
//       });
...
//         alert(
//           "Success! Your plan has been locked in. Our team will contact you shortly.",
//         );
...
//           Secure strategically located farmland at BugaKing Farmers Estate.
//           Tailored payment plans designed for your agricultural goals.
//         </p>
//       </header>
...
//               <p className="text-xs text-white/60">Verified documentation</p>
//             </div>
...
//               Select your desired number of plots and explore our flexible
//               outright, monthly, or weekly payment options.
//             </p>
//           </div>
...
//                   {/* --- COMPLIANT OUTRIGHT BONUS CALLOUT --- */}
//                   <div
//                     className={`mt-4 p-4 rounded-xl border transition-all duration-300 flex items-start gap-3 ${plan === "outright" ? "bg-[#d4af37]/10 border-[#d4af37]/30" : "bg-slate-50 border-slate-100 opacity-60"}`}
//                   >
//                     <span
//                       className={`material-icons-round text-xl shrink-0 ${plan === "outright" ? "text-[#d4af37]" : "text-slate-400"}`}
//                     >
//                       workspace_premium
//                     </span>
...
//                         Outright Bonus:
//                       </strong>{" "}
//                       Pay outright to qualify for premium annual crop yields and
...
//                       Your land is backed by verifiable documentation (Right of
//                       Occupancy) to guarantee your peace of mind.
//                     </p>
//                   </div>
//                 </div>
//               </div>
...
//                       className="w-full group bg-[#0ff035] hover:bg-[#0bc42a] disabled:bg-slate-500 disabled:cursor-not-allowed transition-all duration-300 text-[#0a2f1c] font-bold py-4 px-6 rounded-lg flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(15,240,53,0.3)] hover:shadow-lg hover:-translate-y-0.5"
//                     >
//                       <span className="uppercase tracking-widest text-xs sm:text-sm text-black whitespace-nowrap">
//                         {isSubmitting ? "Processing..." : "Lock in this plan"}
//                       </span>
//                     </button>
//                   </form>
//                   <p className="text-center text-[10px] text-white/30 mt-3 leading-tight">
//                     By submitting, our team will reach out via WhatsApp with
//                     your official subscription forms and payment details.
//                   </p>
//                 </div>
//               </div>
//             </div>
...
//                 Your investment is highly secure. The estate comes with AGIS
//                 Recertification and an R of O (Right of Occupancy), meaning the
//                 land is officially registered and recognized by the government.
//               </div>
//             </details>
//             <details className="group bg-white rounded-lg border border-slate-200 overflow-hidden">
//               <summary className="flex justify-between items-center font-medium cursor-pointer list-none p-5 text-[#0a2f1c]">
...
//               </summary>
//               <div className="text-slate-600 p-5 pt-0 text-sm leading-relaxed">
//                 No, this estate is designated **strictly for farming**. This
//                 zoning ensures a dedicated, productive agricultural environment
//                 free from residential disruptions.
//               </div>
//             </details>
//             <details className="group bg-white rounded-lg border border-slate-200 overflow-hidden">
//               <summary className="flex justify-between items-center font-medium cursor-pointer list-none p-5 text-[#0a2f1c]">
...
//               </summary>
//               <div className="text-slate-600 p-5 pt-0 text-sm leading-relaxed">
//                 Absolutely! We organize regular site visits for investors to
//                 Kuje, Abuja. You can also schedule a private inspection tour by
...
//               Secure your 500sqm plots today and lock in our flexible payment
//               plans.
//             </p>
//             <button
//               onClick={() => {
//                 /* Consider updating this one too if you want it to jump to the form! */
//               }}
...
import React, { useState } from "react";
import Image from "next/image";
import { Shield, ShieldCheck, Sprout, Tractor, CheckCircle } from "lucide-react";
...
export default function Main() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Send the lead data to your Next.js API route
      const response = await fetch("/api/leads", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      if (response.ok) {
        // Show the UI success state instead of an alert
        setIsSuccess(true);
      } else {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="bg-[#0a2f1c] rounded-xl p-8 text-white relative overflow-hidden flex flex-col gap-6 shadow-2xl">
                ...
                <div className="relative z-10 pt-4 min-h-[160px]">
                  {isSuccess ? (
                    // --- SUCCESS STATE UI ---
                    <div className="bg-[#0ff035]/10 border border-[#0ff035]/30 p-8 rounded-xl text-center space-y-3 animate-in fade-in zoom-in duration-300">
                      <CheckCircle className="w-14 h-14 text-[#0ff035] mx-auto" />
                      <h3 className="text-2xl font-bold text-white font-['Playfair_Display',_serif]">Request Received</h3>
                      <p className="text-sm text-white/80 leading-relaxed">
                        Thank you, {fullName || "investor"}. Your details have been securely recorded. Our team will reach out to you shortly via WhatsApp with the official subscription forms.
                      </p>
                    </div>
                  ) : (
                    // --- FORM STATE UI ---
