      ...
      <div className="flex items-start gap-4 flex-1 w-full">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 text-[10px] sm:text-xs text-[#171512]/50 font-bold uppercase tracking-wide">
            ...
            <span className="hidden sm:inline opacity-30">|</span>
            <span className="bg-[#171512]/5 px-2 py-0.5 rounded text-[#171512]/70 lowercase">
              {doc.file_type}
            </span>
          </div>
        </div>
...
// ==========================================
const Main = ({ initialDocuments = [] }: { initialDocuments: DocumentItem[] }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [activeFilter, setActiveFilter] = useState("all");
  const [showMobileFilters, setShowMobileFilters] = useState(false);
  ...
  const stats = useMemo(() => ({
    ...
    agreements: initialDocuments.filter(d => d.category === "agreement").length,
    deeds: initialDocuments.filter(d => d.category === "deed").length,
    reports: initialDocuments.filter(d => d.category === "report").length,
  }), [initialDocuments]);
  ...
  const filteredDocuments = useMemo(() => {
    return initialDocuments.filter((doc) => {
      const matchesSearch = doc.title.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesFilter = activeFilter === "all" || doc.category === activeFilter.slice(0, -1);
      return matchesSearch && matchesFilter;
    });
  }, [initialDocuments, searchQuery, activeFilter]);

  // Grouped results for the UI
  const groups = [
    { label: "Investment Agreements", data: filteredDocuments.filter(d => d.category === "agreement") },
    { label: "Property Deeds", data: filteredDocuments.filter(d => d.category === "deed") },
    { label: "Financial Reports", data: filteredDocuments.filter(d => d.category === "report") },
    { label: "Other Documents", data: filteredDocuments.filter(d => d.category === "other") },
  ];
      ...
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-8 pt-24 lg:pt-10">
        ...
        <div className="mb-8 lg:mb-12">
          <div className="max-w-2xl mb-8">
            <p className="text-[#d0a539] text-[10px] font-black uppercase tracking-[0.4em] mb-2">
              Secure Vault
            </p>
            <h1 className="text-2xl lg:text-4xl font-black text-[#171512] tracking-tight uppercase mb-3">
              Your Document Center
            </h1>
            <p className="text-[#171512]/60 text-sm sm:text-base leading-relaxed">
              Secure access to your verified investment documents and property records.
            </p>
          </div>

          {/* Stats Cards */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatsCard icon={FolderOpen} label="Total Files" value={stats.total} />
            <StatsCard icon={FileText} label="Agreements" value={stats.agreements} />
            <StatsCard icon={Building} label="Deeds" value={stats.deeds} />
            <StatsCard icon={BarChart} label="Reports" value={stats.reports} />
          </div>

          {/* Toolbar */}
          <div className="bg-white p-4 rounded-2xl border border-[#171512]/5 flex flex-col md:flex-row gap-4 shadow-sm">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#171512]/40 w-5 h-5" />
              <input
                className="w-full bg-[#f8f7f6] border-none rounded-xl pl-12 pr-4 py-3 text-[#171512] text-sm focus:ring-1 focus:ring-[#d0a539] outline-none"
            ...
            <div className="hidden md:block relative min-w-[220px]">
              <select
                className="w-full bg-[#f8f7f6] border-none rounded-xl pl-4 pr-10 py-3 text-[#171512] text-sm font-bold uppercase tracking-widest focus:ring-1 focus:ring-[#d0a539] outline-none cursor-pointer appearance-none"
                value={activeFilter}
                onChange={(e) => setActiveFilter(e.target.value)}
              >
                <option value="all">All Documents</option>
            ...
            <button
              className="md:hidden bg-[#171512] text-white px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"
              onClick={() => setShowMobileFilters(!showMobileFilters)}
            >
        ...
        <div className="space-y-12">
          {filteredDocuments.length === 0 ? (
            <div className="bg-white rounded-3xl p-20 border border-[#171512]/5 text-center shadow-sm">
              <FolderOpen className="w-12 h-12 text-[#171512]/10 mx-auto mb-4" />
              <h3 className="text-lg font-black text-[#171512] mb-1">No documents found</h3>
              <p className="text-[#171512]/50 text-sm">Your secure archive is empty or matches no search.</p>
            </div>
          ) : (
            groups.map((group) => group.data.length > 0 && (
              <div key={group.label} className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="flex items-center gap-4">
                  <h3 className="text-[#171512]/30 text-[10px] font-black uppercase tracking-[0.3em] whitespace-nowrap">
                    {group.label}
                  </h3>
                  <div className="h-px flex-1 bg-[#171512]/5"></div>
                </div>
                <div className="grid gap-4">
                  {group.data.map((doc) => <DocumentRow key={doc.id} doc={doc} />)}
                </div>
              </div>
            ))
          )}
        ...
        <div className="mt-20 bg-[#171512] rounded-[2.5rem] p-8 lg:p-14 text-white relative overflow-hidden shadow-2xl">
          <div className="absolute top-0 right-0 w-1/3 h-full bg-[#d0a539]/10 -skew-x-12 translate-x-1/4 pointer-events-none"></div>
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="max-w-xl space-y-4 text-center md:text-left">
              <span className="text-[#d0a539] text-[10px] font-black uppercase tracking-[0.4em]">Investor Support</span>
              <h2 className="text-2xl md:text-4xl font-serif font-bold leading-tight italic">
                Requiring physical deed delivery?
              </h2>
              <p className="text-white/50 text-sm md:text-base leading-relaxed">
                Our legal department handles physical documentation requests and certified property audits. Contact your account manager for verified hard copies.
              </p>
            </div>
            <button className="whitespace-nowrap bg-[#d0a539] text-[#171512] px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white transition-all shadow-xl shadow-[#d0a539]/20">
              Request Hard Copy
            </button>
          </div>
        </div>
